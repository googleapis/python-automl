...
InputConfig = _reflection.GeneratedProtocolMessageType(
  ...
  The format of input depends on dataset\_metadata the Dataset into
  ...
  **In-line JSONL files**  In-line .JSONL files contain, per line, a
  ...
  "end_offset": 7}             }           }        ]     }  **JSONL
  files that reference documents**  .JSONL files contain, per line, a
  JSON document that wraps a ``input_config`` that contains the path to
  a source document. Multiple JSON documents can be separated using line
  ...
  }         }       }     }  **In-line JSONL files with document layout
  information**  **Note:** You can only annotate documents using the UI.
  The format described below applies to annotated documents exported
  using the UI or ``exportData``.  In-line .JSONL files for documents
  contain, per line, a JSON document that wraps a ``document`` field
  that provides the textual content of the document and the layout
  information.  For example:  ::      {       "document": {
  "document_text": {                 "content": "dog car cat"
  }               "layout": [                 {
  "text_segment": {                     "start_offset": 0,
  "end_offset": 11,                    },
  "page_number": 1,                    "bounding_poly": {
  ...
  <section>  .. raw:: html     <h5>  Classification  .. raw:: html
  </h5>  See `Preparing your training data
  <https://cloud.google.com/natural-language/automl/docs/prepare>`__ for
  more information.  One or more CSV file(s) with each line in the
  following format:  ::      ML_USE,(TEXT_SNIPPET |
  GCS_FILE_PATH),LABEL,LABEL,...  -  ``ML_USE`` - Identifies the data
  set that the current row (file)    applies to. This value can be one
  of the following:     -  ``TRAIN`` - Rows in this file are used to
  train the model.    -  ``TEST`` - Rows in this file are used to test
  the model during       training.    -  ``UNASSIGNED`` - Rows in this
  file are not categorized. They are       Automatically divided into
  train and test data. 80% for training       and 20% for testing.  -
  ...
  in size.     For the ``MULTICLASS`` classification type, at most one
  ``LABEL`` is    allowed.     The ``ML_USE`` and ``LABEL`` columns are
  optional. Supported file    extensions: .TXT, .PDF, .TIF, .TIFF, .ZIP
  A maximum of 100 unique labels are allowed per CSV row.  Sample rows:
  ...
  and 3 may be large.  Sample rows:  ::      TRAIN,"@freewrytin this is
  way too good for your product",2     gs://folder/content.txt,3
  ...
  [gcs\_source][google.cloud.automl.v1.InputConfig.gcs\_source] or [bigq
  uery\_source][google.cloud.automl.v1.InputConfig.bigquery\_source].
  All input is concatenated into a single  [primary\_table\_spec\_id][go
  ogle.cloud.automl.v1.TablesDatasetMetadata.primary\_table\_spec\_id]
  **For gcs\_source:**  CSV file(s), where the first row of the first
  ...
  sample rows of a CSV file:  .. raw:: html     <pre>    "Id","First
  Name","Last Name","Dob","Addresses"     "1","John","Doe","1968-01-22",
  "[{"status":"current","address":"123_First_Avenue","city":"Seattle","s
  tate":"WA","zip":"11111","numberOfYears":"1"},{"status":"previous","ad
  dress":"456_Main_Street","city":"Portland","state":"OR","zip":"22222",
  "numberOfYears":"5"}]"     "2","Jane","Doe","1980-10-16","[{"status":"
  current","address":"789_Any_Avenue","city":"Albany","state":"NY","zip"
  :"33333","numberOfYears":"2"},{"status":"previous","address":"321_Main
  _Street","city":"Hoboken","state":"NJ","zip":"44444","numberOfYears":"
  3"}]}    </pre>  **For bigquery\_source:**  An URI of a BigQuery
  table. The user data size of the BigQuery table must be 100GB or
  smaller.  An imported table must have between 2 and 1,000 columns,
  inclusive, and between 1000 and 100,000,000 rows, inclusive. There are
  at most 5 import data running in parallel.  .. raw:: html
  </section>  .. raw:: html     </div>  **Input field definitions:**
  ``ML_USE``     ("TRAIN" \| "VALIDATE" \| "TEST" \| "UNASSIGNED")
  Describes how the     given example (file) should be used for model
  training. "UNASSIGNED"     can be used when user has no preference.
  ``GCS_FILE_PATH``     The path to a file on Google Cloud Storage. For
  example,     "gs://folder/image1.png". ``LABEL``     A display name of
  an object on an image, video etc., e.g. "dog".     Must be up to 32
  characters long and can consist only of ASCII Latin     letters A-Z
  and a-z, underscores(\_), and ASCII digits 0-9. For each     label an
  AnnotationSpec is created which display\_name becomes the     label;
  ...
  vertices of the rectangle. ``VERTEX``     (``COORDINATE,COORDINATE``)
  ...
  left. ``TIME_SEGMENT_START``     (``TIME_OFFSET``) Expresses a
  beginning, inclusive, of a time     segment within an example that has
  a time dimension (e.g. video). ``TIME_SEGMENT_END``
  (``TIME_OFFSET``) Expresses an end, exclusive, of a time segment
  within n example that has a time dimension (e.g. video).
  ``TIME_OFFSET``     A number of seconds as measured from the start of
  an example (e.g.     video). Fractions are allowed, up to a
  microsecond precision. "inf"     is allowed, and it means the end of
  the example. ``TEXT_SNIPPET``     The content of a text snippet, UTF-8
  encoded, enclosed within double     quotes (""). ``DOCUMENT``     A
  field that provides the textual content with document and the
  layout information.  **Errors:**  If any of the provided CSV files
  can't be parsed or if more than certain percent of CSV rows cannot be
  processed then the operation fails and nothing is imported. Regardless
  of overall success or failure the per-row failures, up to a certain
  count cap, is listed in Operation.metadata.partial\_failures.
  Attributes:
      source:
...
BatchPredictInputConfig = _reflection.GeneratedProtocolMessageType(
  ...
  near the end of this comment. The formats are:  .. raw:: html     <h4>
  AutoML Vision  .. raw:: html     </h4>  .. raw:: html     <div
  class="ds-selector-tabs">  .. raw:: html     <section>  .. raw:: html
  ...
  "content": "Extended sample content",          "mime_type":
  "text/plain"        }      }  Sample document JSONL file (Shown with
  artificial line breaks. Actual line breaks are denoted by ":raw-
  latex:`\n`".):  ::       {        "document": {
  "input_config": {            "gcs_source": { "input_uris": [
  "gs://folder/document1.pdf" ]            }          }        }
  }\\n      {        "document": {          "input_config": {
  "gcs_source": { "input_uris": [ "gs://folder/document2.tif" ]
  }          }        }      }  .. raw:: html     </section>  .. raw::
  html     </div>  .. raw:: html     <h4>  AutoML Tables  .. raw:: html
  </h4>  .. raw:: html     <div class="ui-datasection-main">  .. raw::
  html     <section class="selected">  See `Preparing your training data
  <https://cloud.google.com/automl-tables/docs/predict-batch>`__ for
  more information.  You can use either [gcs\_source][google.cloud.autom
  l.v1.BatchPredictInputConfig.gcs\_source] or
  [bigquery\_source][BatchPredictInputConfig.bigquery\_source].  **For
  gcs\_source:**  CSV file(s), each by itself 10GB or smaller and total
  ...
  must contain the model's  [input\_feature\_column\_specs'][google.clou
  d.automl.v1.TablesModelMetadata.input\_feature\_column\_specs]
  [display\_name-s][google.cloud.automl.v1.ColumnSpec.display\_name]
  ...
  Years":"3"}]}    </pre>  **For bigquery\_source:**  The URI of a
  BigQuery table. The user data size of the BigQuery table must be 100GB
  or smaller.  The column names must contain the model's  [input\_featur
  e\_column\_specs'][google.cloud.automl.v1.TablesModelMetadata.input\_f
  eature\_column\_specs]
  [display\_name-s][google.cloud.automl.v1.ColumnSpec.display\_name]
  (order doesn't matter). The columns corresponding to the model's input
  feature column specs must contain values compatible with the column
  ...
  **Input field definitions:**  ``GCS_FILE_PATH``     The path to a file
  ...
  that has a time dimension (e.g. video). ``TIME_OFFSET``     A number
...
OutputConfig = _reflection.GeneratedProtocolMessageType(
  ...
  BigQuery. Google Cloud Storage case:  [gcs\_destination][google.cloud.
  automl.v1p1beta.OutputConfig.gcs\_destination] must be set. Exported
  are CSV file(s) ``tables_1.csv``, ``tables_2.csv``,...,\
  ``tables_N.csv`` with each having as header line the table's column
  names, and all other lines contain values for the header columns.
  BigQuery case:  [bigquery\_destination][google.cloud.automl.v1p1beta.O
  utputConfig.bigquery\_destination] pointing to a BigQuery project must
  be set. In the given project a new dataset will be created with name
  ...
  Attributes:
      ...
      gcs_destination:
          ...
          export\_data-- where timestamp is in YYYY-MM-DDThh:mm:ss.sssZ
...
BatchPredictOutputConfig = _reflection.GeneratedProtocolMessageType(
  ...
  s\_destination][google.cloud.automl.v1.BatchPredictOutputConfig.gcs\_d
  estination] must be set unless specified otherwise for a domain. If
  gcs\_destination is set then in the given directory a new directory is
  ...
  once with all its annotations, and its        annotations will never
  be split across files.         If prediction for any input file (or
  ...
  for any input file (or document) failed (partially or
  ...
  text\_extraction detail populated and refer, via their    indices, to
  ...
  ion][google.cloud.automl.v1p1beta.BatchPredictOutputConfig.gcs\_destin
  ation] or  [bigquery\_destination][google.cloud.automl.v1p1beta.BatchP
  redictOutputConfig.bigquery\_destination] is set (either is allowed).
  ...
  successfully predicted rows. For all CLASSIFICATION  [prediction\_type
  -s][google.cloud.automl.v1p1beta.TablesModelMetadata.prediction\_type]
  : Each .csv file will contain a header, listing all columns'  [display
  \_name-s][google.cloud.automl.v1p1beta.ColumnSpec.display\_name] given
  on input followed by M target column names in the format of  "<[target
  \_column\_specs][google.cloud.automl.v1p1beta.TablesModelMetadata.targ
  et\_column\_spec]  [display\_name][google.cloud.automl.v1p1beta.Column
  Spec.display\_name]>\_\_score" where M is the number of distinct
  ...
  respective values of successfully predicted rows, with the last, i.e.
  the target, columns having the corresponding prediction
  ...
  1p1beta.TablesModelMetadata.prediction\_type]: Each .csv file will
  contain a header, listing all columns'
  [display\_name-s][google.cloud.automl.v1p1beta.display\_name] given on
  input followed by the predicted target column with name in the format
  of  "predicted\_<[target\_column\_specs][google.cloud.automl.v1p1beta.
  TablesModelMetadata.target\_column\_spec]  [display\_name][google.clou
  d.automl.v1p1beta.ColumnSpec.display\_name]>" Subsequent lines will
  ...
  y\_destination][google.cloud.automl.v1p1beta.OutputConfig.bigquery\_de
  stination] pointing to a BigQuery project must be set. In the given
  ...
  display\_name-s][google.cloud.automl.v1p1beta.ColumnSpec.display\_name
  ] followed by the target column with name in the format of  "predicted
  \_<[target\_column\_specs][google.cloud.automl.v1p1beta.TablesModelMet
  adata.target\_column\_spec]  [display\_name][google.cloud.automl.v1p1b
  eta.ColumnSpec.display\_name]>" The input feature columns will contain
  the respective values of successfully predicted rows, with the target
  ...
  format of  "errors\_<[target\_column\_specs][google.cloud.automl.v1p1b
  eta.TablesModelMetadata.target\_column\_spec]  [display\_name][google.
  cloud.automl.v1p1beta.ColumnSpec.display\_name]>", and as a value has
  ```google.rpc.Status`` <https:%20//github.com/googleapis/googleapis/bl
...
ModelExportOutputConfig = _reflection.GeneratedProtocolMessageType(
  ...
  Attributes:
      ...
      model_format:
          ...
          "tflite", "tf\_saved\_model", "tf\_js".    Formats
          description:  -  tflite - Used for Android mobile devices. -
          edgetpu\_tflite - Used for `Edge    TPU
