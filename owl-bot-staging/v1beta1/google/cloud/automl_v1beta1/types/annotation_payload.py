...
class AnnotationPayload(proto.Message):
    ...
    translation = proto.Field(proto.MESSAGE, number=2, oneof='detail',
        message=gca_translation.TranslationAnnotation,
    )

    classification = proto.Field(proto.MESSAGE, number=3, oneof='detail',
        message=gca_classification.ClassificationAnnotation,
    )

    image_object_detection = proto.Field(proto.MESSAGE, number=4, oneof='detail',
        message=detection.ImageObjectDetectionAnnotation,
    )

    video_classification = proto.Field(proto.MESSAGE, number=9, oneof='detail',
        message=gca_classification.VideoClassificationAnnotation,
    )

    video_object_tracking = proto.Field(proto.MESSAGE, number=8, oneof='detail',
        message=detection.VideoObjectTrackingAnnotation,
    )

    text_extraction = proto.Field(proto.MESSAGE, number=6, oneof='detail',
        message=gca_text_extraction.TextExtractionAnnotation,
    )

    text_sentiment = proto.Field(proto.MESSAGE, number=7, oneof='detail',
        message=gca_text_sentiment.TextSentimentAnnotation,
    )

    tables = proto.Field(proto.MESSAGE, number=10, oneof='detail',
