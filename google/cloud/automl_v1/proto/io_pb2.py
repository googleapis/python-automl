...
InputConfig = _reflection.GeneratedProtocolMessageType(
  ...
  unless specified otherwise. Additionally any input .CSV file by itself
  must be 100MB or smaller, unless specified otherwise. If an “example”
  file (that is, image, video etc.) with identical content (even if it had
  different ``GCS_FILE_PATH``) is mentioned multiple times, then its
  label, bounding boxes etc. are appended. The same file should be always
  provided with the same ``ML_USE`` and ``GCS_FILE_PATH``, if it is not,
  ...
  **Input field definitions:**
  ...
  ``LABEL``
     A display name of an object on an image, video etc., e.g. “dog”. Must
  ...
  ``TIME_OFFSET``
     A number of seconds as measured from the start of an example
     (e.g. video). Fractions are allowed, up to a microsecond precision.
     “inf” is allowed, and it means the end of the example.
  ``TEXT_SNIPPET``
  ...
  **For bigquery_source:**
  ...
  types. Prediction on all the rows of the table will be attempted.
...
OutputConfig = _reflection.GeneratedProtocolMessageType(
  ...
  each line in format: ML_USE,GCS_FILE_PATH GCS_FILE_PATH leads to a .TSV
  file which describes examples that have given ML_USE, using the
...
BatchPredictOutputConfig = _reflection.GeneratedProtocolMessageType(
  ...
  must be set unless specified otherwise for a domain. If gcs_destination
  ...
  Classification: In the created directory a video_classification.csv
  ...
  where: GCS_FILE_PATH,TIME_SEGMENT_START,TIME_SEGMENT_END = matches 1 to
  1 the prediction input lines (i.e. video_classification.csv has
  ...
  -  For Video Object Tracking: In the created directory a
     ...
     video_object_trackinng_N.json, where N is the number of requests in
     the input (i.e. the number of lines in given CSV(s)).
  ...
  GCS_FILE_PATH,TIME_SEGMENT_START,TIME_SEGMENT_END,JSON_FILE_NAME,STATUS
  where: GCS_FILE_PATH,TIME_SEGMENT_START,TIME_SEGMENT_END = matches 1 to
  1 the prediction input lines (i.e. video_object_tracking.csv has
  precisely the same number of lines as the prediction input had.)
  JSON_FILE_NAME = Name of .JSON file in the output directory, which
  contains prediction responses for the video time segment. STATUS = “OK”
  if prediction completed successfully, or an error code with message
  ...
  tables will be created, ``predictions``, and ``errors``. The
