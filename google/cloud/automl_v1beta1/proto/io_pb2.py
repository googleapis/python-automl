...
InputConfig = _reflection.GeneratedProtocolMessageType(
  ...
  -  For Image Classification: CSV file(s) with each line in format:
     ML_USE,GCS_FILE_PATH,LABEL,LABEL,… GCS_FILE_PATH leads to image of up
     to 30MB in size. Supported extensions: .JPEG, .GIF, .PNG, .WEBP,
  ...
  -  For Text Extraction: CSV file(s) with each line in format:
     ...
     “input_uris”: [ “gs://folder/document1.pdf” ] } } } }:raw-latex:`\n
           {
             "document": {
               "input_config": {
  ...
  -  For Text Classification: CSV file(s) with each line in format:
     ...
     is a valid gcs file path, i.e. prefixed by “gs://”, it will be
     treated as a GCS_FILE_PATH, else if the content is enclosed within
  ...
  one. The SENTIMENT shouldn’t be confused with”score" or “magnitude” from
  the previous Natural Language Sentiment Analysis API. All SENTIMENT
...
BatchPredictInputConfig = _reflection.GeneratedProtocolMessageType(
  ...
  -  For Text Classification: CSV file(s) with each line having just a
     single column: GCS_FILE_PATH \| TEXT_SNIPPET Any given text file can
     have size upto 128kB. Any given text snippet content must have 60,000
  ...
  -  For Text Sentiment: CSV file(s) with each line having just a single
     column: GCS_FILE_PATH \| TEXT_SNIPPET Any given text file can have
  ...
  -  For Text Extraction .JSONL (i.e. JSON Lines) file(s) which either
     ...
     input_config set. Only PDF documents are supported now, and each
...
BatchPredictOutputConfig = _reflection.GeneratedProtocolMessageType(
  ...
  ``tables_1.csv``, ``tables_2.csv``,…, ``tables_N.csv`` will be created,
  where N may be 1, and depends on the total number of the successfully
  predicted rows. For all CLASSIFICATION
...
GcrDestination = _reflection.GeneratedProtocolMessageType(
  ...
  Attributes:
      output_uri:
          ...
          to 2000 characters long. See  https: //cloud.google.com
          /container-registry/do // cs/pushing-and-
